# Model B - Goal 2
# Predicts whether a customer ever converts to a paid membership
...
# load data
print("=" * 60)
print("MODEL B - Predict membership conversion")
print("=" * 60)
...
# flag visits with membership pricing
# captures Black Level (unlimited), Gray Level (8/mo), Platinum/Yearly
# excludes Single Session Bundles which are NOT memberships
pricing_str = d3['pricing_option'].astype(str)
bundle_mask = pricing_str.str.contains('Single Session Bundle', case=False, na=False)
member_mask = pricing_str.str.contains(
    r'\bLevel\b|Unlimited|Per Month|/Month|Membership|Yearly',
    case=False, na=False, regex=True
) & ~bundle_mask
d3['is_member_visit'] = member_mask.astype(int)
...
print("Cohort cutoff:", cutoff_30d.date())


# find first visit per (client_id, facility)
...
# cohort: first visit 2022+ and on or before cutoff
...
# membership target: customer ever had a membership-tagged visit at this facility
ever_member = d3.groupby(['client_id', 'facility'])['is_member_visit'].max().reset_index()
ever_member.columns = ['client_id', 'facility', 'ever_member']
cohort = cohort.merge(ever_member, on=['client_id', 'facility'], how='left')
cohort['ever_member'] = cohort['ever_member'].fillna(0).astype(int)


# IMPORTANT - exclude membership visits from the first-30-day feature window
# if a customer's first visit was on a membership, that means they bought
# the membership before their first session, which makes early-behavior
# features circular with the target
# we already filtered for signed-in visits so this only affects feature counts
# nothing changes here since visits_first_30d counts attendance, not pricing
# but flag it for the writeup
...
print("Membership rate:", round(cohort['ever_member'].mean() * 100, 1), "%")
print("Total members:", cohort['ever_member'].sum())


# replace less common first-visit coaches with 'Other'
...
def build_models(df_subset, version_name, include_facility=True):
    ...
    print("Members:", df_subset['ever_member'].sum(),
          "(", round(df_subset['ever_member'].mean() * 100, 1), "%)")
    ...
    print("Members in test set:", y_test.sum())

    metrics = {'version': version_name, 'n_rows': len(df_subset),
               'n_features': X.shape[1], 'n_members': df_subset['ever_member'].sum()}
...
# final summary
print("=" * 60)
print("MODEL B - MEMBERSHIP CONVERSION COMPARISON")
print("=" * 60)
